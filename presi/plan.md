...
- Stage 4 is implemented.  `make -C presi -j 8 cosim` builds the **co-simulation binary** `_build/presi-gates-cosim` (~124 MB at the default `-Og`; clean rebuild ~5:30) which links the abr_wrap netlist + standalone gate netlists for `ntt_top` and `abr_sampler_top` plus per-engine glue (`<engine>.glue.c`, generated by `gen_engine_glue.py`).  `make run-cosim` runs the smoke harness; after 64 reset cycles plus 64 idle cycles the harness reads `MLDSA_CORE_NAME[31:0]=0x44534d4c`, `MLDSA_CORE_NAME[63:32]=0x3837412d`, `MLDSA_CORE_VERSION[31:0]=0x302e322e`, `MLDSA_CORE_VERSION[63:32]=0x00003300`, and `STATUS=1` (READY bit set).  Writing `MLDSA_CTRL=1` then walks the abr_seq ROM through real Dilithium-keygen UOPs (`pc=2 LD_SHAKE256 → pc=3 SHAKE256 → pc=4 LFSR → pc=5 SHAKE256 → pc=6 REJB s1[0] → pc=7 REJB s1[1]`) within the 256-cycle smoke poll window; engine handshakes (sampler_busy_o, SHA3 absorb/squeeze, rejection-sampling) all execute through gate-level netlists.  Run cost **~8.5 cyc/s** at `-Og` (smoke 384 cy in 0:45 wall) -- 3.7× faster than `-O0`'s 2.3 cyc/s.  See `Makefile`'s `GATES_OPT` comment for the full -O0/-Og/-O1 measurement matrix.
  ...
  What remains:
  - End-to-end Dilithium / ML-KEM keygen byte-compare against Verilator (Stage 5).  ML-KEM keygen presi vs Verilator: cycle-perfect for pc=443..462, then **the standalone-elaborated `ntt_top` runs its NTT and PWA modes with a shortened internal pipeline** -- pc=462 NTT(e[3]) takes 230 cy vs Verilator's 309 (-79 cy), pc=467 PWA takes only **3 cy** vs Verilator's 70 (-67 cy), and pc=468 then deadlocks because the truncated PWA leaves PWM's internal counters in a state that can never advance to RD_IDLE/WR_IDLE (`ntt_busy=1 sampler_busy=0` forever).  Diagnosis localized 2026-05-12 with cycle-by-cycle `[eng]` traces (`./abr_wrap +trace-eng` and `./_build/presi-gates-cosim -trace-eng`); ground-truth logs at `_build/{presi,verilator}_eng_mlkem-keygen.log`.  Diff helper: `tools/quick_diff.sh`.  Engine handshake printf lives in `presi_gates.c::presi_eng_trace_step`; signals come from named flop indices in `_build/{ntt_top,abr_sampler_top}.presi_map.csv`.  Confirmed NOT the SRAM truncation (fix landed 2026-05-09 ;`bb_wiring.h` samples full widths; cycle counts identical pre- and post-fix), NOT `$mem_v2` cells surviving (both engine stat .rpts show zero), NOT the engine engagement gate (reverted; see [[engine_gate_dead_end]]).  Root cause is inside `ntt_ctrl.sv`'s `read_fsm` / `write_fsm` counter behavior under standalone Yosys elaboration; next step is to add named-flop traces for `read_fsm_state_ps`, `write_fsm_state_ps`, `pwo_mode`, `pwo_busy` and bisect within pc=467 to find the exact cycle where the FSM state diverges from what it should be (1 hour of work, just extending the existing `_IDX` table in `presi_gates.c`).
