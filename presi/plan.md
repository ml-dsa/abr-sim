...
- Stage 4 is implemented.  `make -C presi -j 8 cosim` builds the **co-simulation binary** `_build/presi-gates-cosim` (~124 MB at the default `-Og`; clean rebuild ~5:30) which links the abr_wrap netlist + standalone gate netlists for `ntt_top` and `abr_sampler_top` plus per-engine glue (`<engine>.glue.c`, generated by `gen_engine_glue.py`).  `make run-cosim` runs the smoke harness; after 64 reset cycles plus 64 idle cycles the harness reads `MLDSA_CORE_NAME[31:0]=0x44534d4c`, `MLDSA_CORE_NAME[63:32]=0x3837412d`, `MLDSA_CORE_VERSION[31:0]=0x302e322e`, `MLDSA_CORE_VERSION[63:32]=0x00003300`, and `STATUS=1` (READY bit set).  Writing `MLDSA_CTRL=1` then walks the abr_seq ROM through real Dilithium-keygen UOPs (`pc=2 LD_SHAKE256 → pc=3 SHAKE256 → pc=4 LFSR → pc=5 SHAKE256 → pc=6 REJB s1[0] → pc=7 REJB s1[1]`) within the 256-cycle smoke poll window; engine handshakes (sampler_busy_o, SHA3 absorb/squeeze, rejection-sampling) all execute through gate-level netlists.  Run cost **~8.5 cyc/s** at `-Og` (smoke 384 cy in 0:45 wall) -- 3.7× faster than `-O0`'s 2.3 cyc/s.  See `Makefile`'s `GATES_OPT` comment for the full -O0/-Og/-O1 measurement matrix.
  ...
  - **abr_seq synchronous-ROM model -- fixed 2026-05-12.**  The abr_seq blackbox was being emitted by `gen_blackbox_wiring.py` as a **combinational** ROM (read `addr_i` from current `presi_s[]`, drive `data_o` immediately in the post-settle SRAM tick).  The SV is in fact a synchronous flop (`always_ff @(posedge clk) if (en_i) data_o_rom <= ROM[addr_i]`).  Under the 0-cycle model, any mid-cycle wobble of `abr_prog_cntr_nxt` (e.g. when engine `ntt_busy` transiently dipped to 0 because `mode` was changing in the same comb loop) immediately propagated to `data_o → abr_instr → ntt_mode → mode`, which fed back into the engine's ct_mode/gs_mode test and made `ntt_busy` collapse early.  Mid-NTT mode flap shortened NTT(e[3]) by 79 cy and PWA by 67 cy and deadlocked the first REJS_PWM of row 1.  The fix splits the seq tick out of `presi_sram_tick_all()` into a new `presi_seq_tick()` (generated body `<top>.presi_seq_tick.h`) called between phase-1 comb and step_netlist_flop in `presi_cycle` -- so abr_prog_cntr.D and the seq ROM's "address sample" both latch the same `abr_prog_cntr_nxt` comb value at the rising edge.  Per-instance static slots `_seq_addr_q_<inst>` / `_seq_en_q_<inst>` carry the registered value across cycles, matching the 1-cycle Q := D@edge semantics of the SV flop.  Post-fix, mlkem-keygen pc deltas are exactly Verilator's (0 drift on the 8 NTTs through pc=462, the 4 REJS_PWMA in row 0, REJS_PWM/A in row 1, etc.); ±4 cy boundary drift on the few transitions adjacent to a mode change (e.g. -4 at pc=463 paired with +4 at pc=467) -- these are pure idle wait cycles between operations and do not change what the engines compute.  See `~/.claude/.../engine_cosim_divergence.md` for the post-fix delta table and `_build/presi_eng_v4_post-fix.log` for the captured trace.

  What remains:
  - End-to-end byte-compare presi cosim output (`-ek`, `-dk`) against Verilator's `ek_out.dat` / `dk_out.dat`.  Cycle-delta evidence (see above) makes computational divergence essentially impossible -- engines are deterministic functions of their inputs and take cycle-perfect time on every transition past the initial settle -- but a literal byte-compare is the cleanest single test.  Full mlkem-keygen end-to-end is ~11000 presi cycles ≈ 23 min wall at 8 cy/s; running with `-trace-eng -trace-fsm` disabled and `stdbuf` off saves some wall.
