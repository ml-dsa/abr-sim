...
- Stage 3 is started: `presi/flow/spice_to_c.py` recognizes the BUF/NOT/NAND/NOR/DFF/DFFSR primitive set. It still needs extension to handle the larger `simplemap` set (`$_AND_`, `$_OR_`, `$_XOR_`, `$_XNOR_`, `$_NAND_`, `$_NOR_`, `$_ANDNOT_`, `$_ORNOT_`, `$_MUX_`, `$_DFF_P_`, `$_DFFSR_PPP_`) and the blackbox subcircuit instances (engines + SRAMs + twiddle ROM).
- Stage 4 is started: `presi/presi.c` has reset/cycle/AHB driver scaffolding and C SRAM allocation, but generated top-level port and SRAM port bindings are still TODO.
