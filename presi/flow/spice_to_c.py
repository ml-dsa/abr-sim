...
def write_var_header(path, top, ordered_names):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("/* Last-clock snapshot for edge-triggered DFFs.  The\n"
                " * harness updates this just before each presi_step_netlist\n"
                " * call so the rising-edge predicate `(clk & ~presi_clk_prev)`\n"
                " * fires exactly once per logical cycle. */\n")
        f.write("extern presi_t presi_clk_prev;\n")
...
def write_var_definitions(path, header_basename, ordered_names):
    with open(path, "w", encoding="utf-8") as f:
        f.write("/* Generated by spice_to_c.py.  Do not edit. */\n")
        f.write('#include "%s"\n' % header_basename)
        f.write("presi_t presi_clk_prev;\n")
        for c in ordered_names:
            f.write("presi_t %s;\n" % c)
