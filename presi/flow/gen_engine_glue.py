...
def emit_glue(out_path, engine, instance, prefix, num_parts,
    ...
    """Emit a glue C file with two flavors of step function:

      `<engine>_step_glue_comb(void)`  -- input copy + comb step + output copy.
      `<engine>_step_glue_flop(void)`  -- flop step + clk_prev update + output copy.

    The harness calls _comb on every step_netlist phase (always need
    fresh comb) and _flop only on the rising-edge phase (the only
    time `_edge = clk & ~clk_prev` is non-zero).  Skipping _flop on
    the falling-edge and settle phases avoids ~240k wasted Q := Q
    bitwise ops per cycle in the heavy ntt_top / abr_sampler_top
    parts.

    `_flop` does not redo input copy: it runs immediately after _comb
    in the rising-edge phase, so the engine's input pins are already
    consistent with abr_wrap.  The output copy at the end of _flop
    is required because flop ticks mutate Q values that drive abr_wrap
    bb pins; downstream comb in abr_wrap (and the settle pass) needs
    those fresh.
    """
    ...
    # Pre-build the input-copy and output-copy bodies; the two glue
    # functions share the same pin lists.
    input_lines = []
    output_lines = []
    ...
    # Locate the engine's clk port for the clk_prev update at end of
    # the flop glue.
    clk_eng = None
    for (port, bit, direction, count) in pin_seq:
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("/* Generated by gen_engine_glue.py.  Do not edit.\n"
                " *\n"
                " * Glue between abr_wrap (presi_s[]) and the standalone\n"
                " * %s netlist (%s[]).  Integer indices are baked\n"
                " * in from the two presi_map.csv files; no idx-header\n"
                " * include needed. */\n" % (engine, eng_array))
        ...
        for i in range(num_parts):
            f.write("extern void %spresi_step_part_%03d_comb(presi_t *);\n"
                    % (prefix, i))
        ...
        for i in range(num_parts):
            f.write("\t%spresi_step_part_%03d_comb(%s);\n" %
                    (prefix, i, eng_array))
        f.write("\n\t/* engine outputs -> abr_wrap */\n")
        f.writelines(output_lines)
        if skipped_outputs:
            f.write("\n\t/* outputs tied to constants in abr_wrap "
                    "(unused downstream): */\n")
            for port, bit in skipped_outputs:
                f.write("\t/*   %s[%d] */\n" % (port, bit))
        f.write("}\n")

        # ---- flop glue: step_part_NNN_flop + clk_prev update + output
        # copy.  No input copy: _flop runs immediately after _comb in
        # the rising-edge phase, so the engine inputs are already
        # consistent.  Output copy is mandatory: flop ticks mutate Q
