...
- `make -C presi -j 4 run-gates` — compiles + links the full netlist binary (≈270 MB `presi-gates`) and runs it. ≈3 min on -j 4; needs ≈3 GB per parallel cc1 (a single-function compile is impractical, hence the 32-part split). Reads back the expected `0x44534d4c` ("MLSD") on the abr_wrap NAME register — end-to-end signal path through the netlist works; the 14 engine blackboxes are still stubs.
